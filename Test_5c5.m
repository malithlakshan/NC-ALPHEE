...
for i = 1:length(Hval)
    ...
    for  s = 1:length(sigma)
        ...
        for n_rep = 1:nrep
            ...
            H_sigma(s, n_rep) =   mean(var_H_ests);
            H_est(i,s,n_rep) =  median(var_H_ests);
    ...
    ylabel('$Var(\hat{H})$', 'Interpreter','latex');
    yscale('log')
...
filename = sprintf('./NewFigs/Test_5c5.png');
%saveas(gcf, filename)
...
ylim([0.0 .0015]);
grid on 
filename = sprintf('./NewFigs/Test_5c5_all.png');
saveas(gcf, filename)
