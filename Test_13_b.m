...
for s = 1: length(sigma)
    ...
    for i = 1:length(Hval)        
        ...
        for n_rep = 1:nrep
               ...
               X = [X; [H_ests' sigma(s) H_true ]];  X_var = [X_var; weights' sigma(s) H_true ];
        end
    end
    filename = sprintf( "Malith_Data/Estimated_H_NCALPHEE_%d_sigma_%.2f.csv", m, sigma(s));
    writematrix(X, filename);

    filename = sprintf( "Malith_Data/Estimated_H_NCALPHEE_Weight_%d_sigma_%.2f.csv", m,sigma(s));
    writematrix(X_var, filename);
end
end
